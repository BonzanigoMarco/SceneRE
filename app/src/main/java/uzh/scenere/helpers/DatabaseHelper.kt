...
class DatabaseHelper private constructor(context: Context) {
    ...
    @Suppress("UNCHECKED_CAST")
    public fun <T : Serializable> readBulk(clz: KClass<T>, key: Any? = null, internalMode: DataMode = mode): List<T> {
        when(internalMode){
            DataMode.PREFERENCES -> {
                if (Project::class == clz) return readBulkInternal(clz, PROJECT_UID_IDENTIFIER)
                if (Stakeholder::class == clz) return readBulkInternal(clz, STAKEHOLDER_UID_IDENTIFIER)
                if (Object::class == clz) return readBulkInternal(clz, OBJECT_UID_IDENTIFIER)
                if (Attribute::class == clz) return readBulkInternal(clz, ATTRIBUTE_UID_IDENTIFIER)
                if (Scenario::class == clz) return readBulkInternal(clz, SCENARIO_UID_IDENTIFIER)
            }
            DataMode.DATABASE -> {
                ...
                if (Scenario::class == clz && key is Project) return database!!.readScenarios(key) as List<T>
            }
        }
        return emptyList()
    }
    ...
    private fun <T : Serializable> readBulkInternal(clz: KClass<T>, identifier: String): List<T> {
        ...
        for (entry in sharedPreferences.all) {
            if (entry.key.contains(identifier)) {
                ...
                list.add(DataHelper.toObject(bytes, clz)!!)
            }
        }
